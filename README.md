...
Для проекта предпочтительным выбором базы данных будет реляционная БД PostgreSQL, так как:

1. Она предоставляет структурный подход к хранению данных, где данные организованы в таблицы с определенными связями и ограничениями 
2. СУБД предлагает ACID-транзакции, которые обеспечивают надежность и целостность данных. Такая система гарантирует, что операции записи и обновления данных будут успешно завершены или отменены, а база данных останется в согласованном состоянии даже при сбоях или ошибках.
3. СУБД обладает возможностями горизонтального масштабирования, что позволяет распределить нагрузку на несколько серверов или узлов&

СУБД PostgreSQL будет использоваться для всех таблиц, кроме `Sessions` и `Media`. Для `Sessions` будет использоваться `Redis`, а для `Media` - Amazon S3 (Simple Storage Service), что обеспечит высокую доступность и надежность, а также масштабируемость для обработки огромных объемов данных.
...
Таблица адресов пользователей (у каждого пользователя может быть несколько адресов). Содержит адрес и информацию о получателе.
 ...
 - **Индексы:**
   - `recipient_id` - PK
   - `auther_id` - FK
   - `country` + `region` + `city` + `street` - для быстрого поиска адреса (очень высокая селективность для каждого поля)
 - **Репликация:** master-slave
...
Таблица товаров, котоаря содержит информацию о товаре.
 ...
 - **Индексы:**
   - `product_id` - PK
   - `store_id` - FK
   - `created_at` - для сортировки по времени создания товара
   - `price` - фильтрация по цене
   - `name` - фильтрация по названию
   - `raiting` - фильтрация по рейтингу
 - **Шардирование:** по `store_id`
 - **Денормализация:** допольнительно хранить список id категорий и список id медиа файлов
 - **Репликация:** master-slave
...
Таблица отзывов о товаре.
 ...
 - **Индексы:**
   ...
   - ``user_id`` - FK
   - `product_id` - FK
   - `created_at` - для сортировки по времени создания отзыва
...
Таблица заказов.
 ...
 - **Индексы:**
   ...
   - `recipient_id` - FK
   - `payment_id` - FK
   - `created_at` - для сортировки по времени создания заказа
...
Таблица платежей. 
 ...
 - **Индексы:**
   - `payment_id` - PK
   - `detail_id` - FK
   - `created_at` - для сортировки по времени оплаты
 - **Шардирование:** по `detail_id`
 - **Репликация:** master-master
...
1. Postgres - `pgx` (лучший по бенчмаркам и позволяет настраивать кол-во коннектов к бд)
2. Redis - `go-redis` (популярный клиент Redis для Go)
3. Amazon S3 - `aws-sdk-go` (официальный SDK Amazon Web Services (AWS) для Go. Позволяет работать с Amazon S3 и выполнять операции с бакетами, объектами, доступами и другими функциональностями S3)
