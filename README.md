...
| product_statistics| 266 | 8 * 7 = 56 Б | 14 Гб |
| store_statistics| 0.73 | 8 * 4 = 32 Б | 0.02 Гб |

**Итого:** ~1871 Гб в год или 1.83 Пб (без учета медиа - они были выше)
...
`Stores`, `Products`, `Comments`, `Orders`, `Product_statistics`, `Store_statistics`: Эти таблицы обладают большим размером данных и активно используются, что делает Casandra идеальным выбором из-за ее высокой доступности и производительности. Cassandra также поддерживает шардирование, что позволяет эффективно распределить данные на узлы и обеспечить горизонтальное масштабирование.
...
При работе с хранилищем в микросервисе происходит балансировка на нужный шард (если есть шардирование). Причем стоит отметить некоторые особенности реплицирования:
 ...
 - Для `Users`, `Products`, `Comments` и `Sessions`, `Statistic` используется Master-Slave репликация, где для записи используется Master, а для чтения - Slave.
